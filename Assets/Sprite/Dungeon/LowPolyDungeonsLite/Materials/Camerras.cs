...
public class Camerras : MonoBehaviour
{
    ...
    public float camera_width; //가로거리
    public float camera_height; //세로거리
    public float camera_fix;//레이케스트 후 리그쪽으로 올 거리
    ...
    void Update()
    {
        ...
        RaycastHit hitinfo;
        Physics.Raycast(transform.position, ray_target, out hitinfo, camera_dist,LayerMask.GetMask("Wall"));
        if (hitinfo.transform !=null)//레이케스트 성공시
        {
            //point로 옮긴다.
            MainCamera.transform.position = hitinfo.point;
            //카메라 보정
            MainCamera.transform.Translate(dir * -0.2f * camera_fix);
        }
        else
        {
            ...
            //카메라 보정
            MainCamera.transform.Translate(dir * -0.2f * camera_fix);
            //MainCamera.transform.position = Vector3.Lerp(transform.position, dir, 0.1f);
        ...
        //    MainCamera.transform.Translate(dir * camera_dist);
        //    //카메라 보정
        //    MainCamera.transform.Translate(dir * -0.7f * camera_fix);
        //    //MainCamera.transform.position = Vector3.Lerp(transform.position, dir, 0.1f);
